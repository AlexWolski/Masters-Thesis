...
Implicit representations are memory efficient, detailed, and have no resolution limit. They can be learned efficiently using relatively simple architectures. The main challenge is that the model itself becomes the 3D representation. Algorithms such as Marching Cubes~\cite{Lorensen1987} can be used to convert a learned implicit representation to an explicit format~\cite{Xiao2020}. Doing so, however, negates the advantages of an implicit format such as memory efficiency and infinite resolution.
