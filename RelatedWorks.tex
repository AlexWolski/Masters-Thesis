...
VoxNet~\cite{Maturana2015} improves upon ShapeNet with a 3D CNN architecture. It simple in theory to extend a 2D image processing CNN to handle 3D occupancy grids. In practice, however, the increased memory and computation requirements added by the extra dimension pose a challenge. The VoxNet architecture compensates by using a small input of 32x32x32 voxels. While small compared to other 3D representations, this resolution is superior to that of ShapeNet. The voxel input is fed through a network consisting of two convolution layers, one pooling layer, and two fully-connected layers to produce class label predictions. To better capture fine details, VoxNet implements a multiresolution architecture using two of these convolutional networks. Both networks are identical, including their input dimensions. The difference is in the resolution of the occupancy grid fed to each. The voxels in the coarse input represent a volume of $0.2m^3$ while the voxels of the fine input represent $0.1m^3$. The outputs both networks are then combined to produce a final class prediction. Despite implementing two parallel networks, the VoxNet architecture is much smaller than ShapeNet with more than 10 times fewer parameters. And VoxNet is able to achieve better classification accuracy than ShapeNet in most tests~\cite{Maturana2015}.
