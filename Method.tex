...
A contrastive encoder network is used to extract features from an initial reconstruction input and a target shape input. The contrastive encoder is comprised of two separate encoder networks that share weights. The two feature vectors produced by the contrastive encoder are processed by a contrastive encoder network to find the difference between the reconstruction and target shapes.
